...
val_set_path = '../Dataset/strict_split_fold_{}_validation.csv'.format(fold_num)
test_set_path = '../Dataset/strict_split_test.csv'
...
def StrictRemove(Cell, Drug, IC50, val_csv_path, test_csv_path):
    ...
    for ii in range(len(Drug)):
        if (Cell[ii] not in Cell_) and (Drug[ii] not in Drug_):
...
def GetTrainData(total_csv_path, val_csv_path, test_csv_path):
    ...
    Cell, Drug, IC50 = StrictRemove(Cell, Drug, IC50, val_csv_path, test_csv_path)
